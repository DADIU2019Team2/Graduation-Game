...
public class SmoothPlayerCamFollow : MonoBehaviour
{
    ...
    void moveCam()
    {
        /*var adjustedLocalOffsetX = localOffSet.x * KinematicTestController.runningRight;
        var adjustedLocalOffset = new Vector3(adjustedLocalOffsetX, localOffSet.y, localOffSet.z);*/
    ...
    Vector3 updateDesiredLookAtPointWorld()
    {
        ...
        Vector3 charToFollowVelocity = Quaternion.Inverse(playerToFollow.rotation) * Quaternion.Euler(0,90*KinematicTestController.runningRight,0) * charMotor.Velocity;

        Vector3 dynamicLookAtPoint = new Vector3(0, 0 + worldOffset.y,charToFollowVelocity.x / 2*KinematicTestController.runningRight);
